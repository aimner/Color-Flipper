...
function changeColorScroll(event) {
  let mouseСoordinatesColorPanel = event.clientX - colorScroll.offsetLeft;
  let colorPanelButtonCoordinatsX =
    parseInt(colorPanelButton.style.left) + colorPanelButton.offsetWidth / 2;
  let colorPanelButtonCoordinatsY =
    parseInt(colorPanelButton.style.top) + colorPanelButton.offsetHeight / 2;
  colorScrollControlButton.style.left = `${
    mouseСoordinatesColorPanel - colorScrollControlButton.offsetWidth / 2
  }px`;

  for (let i = 1; i <= 6; i++) {
    if (mouseСoordinatesColorPanel <= gaps[i] && mouseСoordinatesColorPanel >= gaps[i - 1]) {
      rgbNumber = Math.floor(
        ((mouseСoordinatesColorPanel - (colorScroll.offsetWidth / 6) * gaps.indexOf(gaps[i - 1])) *
          255) /
          (colorScroll.offsetWidth / 6)
      );
...
function changeGlobalButtonsArrItem() {
  ...
  globalButtonsArrItem.colorScrollButtonCoordinats = (+colorScrollControlButton.style.left
    .split("")
...
function chooseColorMove() {
  // свайп круглой кнопки для выбора цвета
...
function LOL(event) {
  ...
  if (event.which === 1) {
    ...
    if (
      mouseCoordX > 0 &&
      mouseCoordX <= colorPanel.offsetWidth &&
      mouseCoordY > 0 &&
      mouseCoordY <= colorPanel.offsetHeight
    ) {
      colorPanelButton.style.left = `${mouseCoordX - colorPanelButton.offsetWidth / 2}px`;
      colorPanelButton.style.top = `${mouseCoordY - colorPanelButton.offsetWidth / 2}px`;
      let globalButtonsArrItem = globalButtonsArr.find((item) => item.active === true);
