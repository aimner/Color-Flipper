...
function scrollProcentPanel(item, inputsArr) {
  colorsBlockChooseButtonsArr.forEach((elem) => {
    elem.parentElement.classList.remove("colors_block_element_active");
    if (elem.dataset.number === item.dataset.number) {
      elem.parentElement.classList.add("colors_block_element_active");
    }
  });
  ...
  procentScroll.addEventListener("mousemove", (event) => {
    ...
    if (event.which === 1 && item.classList.contains("procent_scroll_control_button_active")) {
      ...
      if (
        mouseСoordinatesProcentPanel >= 0 &&
        mouseСoordinatesProcentPanel <= procentScroll.offsetWidth
      ) {
...
function changeProcentScroll(event, item, inputsArr) {
  let mouseСoordinatesProcentPanel = event.clientX - procentScroll.offsetLeft;
  item.style.left = `${
    mouseСoordinatesProcentPanel - procentScrollControlButton.offsetWidth / 2
  }px`;
  inputsArr.forEach((input) => {
    if (item.dataset.number === input.dataset.number) {
      input.style.left = `${
        mouseСoordinatesProcentPanel -
        item.offsetWidth / 2 -
        (procentScrollText.offsetWidth / 2 - item.offsetWidth / 2)
      }px`;
      let transparency = (+item.style.left
        .split("")
...
function changeButtonsElement(event) {
  if (event.target.classList.contains("colors_block_choose_button")) {
    ...
    procentScrollButtonsArr.forEach((item) =>
      item.classList.remove("procent_scroll_control_button_active")
    );
    procentScrollButtonsArr
      .find((item) => item.dataset.number === event.target.dataset.number)
      .classList.add("procent_scroll_control_button_active");
  ...
  if (event.target.classList.contains("colors_block_delete_button")) {
    let deleteButtonControl = procentScrollButtonsArr.find(
      (item) => item.dataset.number === event.target.dataset.number
    );
    for (
      let i = 1 + procentScrollButtonsArr.indexOf(deleteButtonControl);
      i < procentScrollButtonsArr.length;
      i++
    ) {
    ...
    let deleteInput = procentScrollInputsArr.find(
      (item) => item.dataset.number === event.target.dataset.number
    );
    for (
      let i = 1 + procentScrollInputsArr.indexOf(deleteInput);
      i < procentScrollInputsArr.length;
      i++
    ) {
    ...
    deleteInput.remove();
    /////////////////////////////////////////
    let deleteGlobalButtonsArrElement = globalButtonsArr.indexOf(
      globalButtonsArr.find((item) => item.dataNumber === +event.target.dataset.number)
    );
    ...
    /////////////////////////////////////////
    let deleteChooseButton = colorsBlockChooseButtonsArr.find(
      (item) => item.dataset.number === event.target.dataset.number
    );
    for (
      let i = 1 + colorsBlockChooseButtonsArr.indexOf(deleteChooseButton);
      i < colorsBlockChooseButtonsArr.length;
      i++
    ) {
      colorsBlockChooseButtonsArr[i].dataset.number =
        +colorsBlockChooseButtonsArr[i].dataset.number - 1;
    ...
    /////////////////////////////////////////
    let deleteDeleteButton = colorsBlockDeleteButtonsArr.find(
      (item) => item.dataset.number === event.target.dataset.number
    );
    for (
      let i = 1 + colorsBlockDeleteButtonsArr.indexOf(deleteDeleteButton);
      i < colorsBlockDeleteButtonsArr.length;
      i++
    ) {
      colorsBlockDeleteButtonsArr[i].dataset.number =
        +colorsBlockDeleteButtonsArr[i].dataset.number - 1;
    ...
    /////////////////////////////////////////
    let deleteExmapleColor = colorsExampleArr.find(
      (item) => item.dataset.number === event.target.dataset.number
    );
    for (
      let i = 1 + colorsExampleArr.indexOf(deleteExmapleColor);
      i < colorsExampleArr.length;
      i++
    ) {
...
function createControlProcentInput(newButton, mouseСoordinatesProcentPanel) {
  ...
  newInput.style.left = `${
    ...
    (procentScrollText.offsetWidth / 2 - newButton.offsetWidth / 2)
  }px`;
  let transparency = (+newInput.style.left
    .split("")
    .filter((item) => !isNaN(+item) || item === ".")
    .join("")).toFixed(0);
  ...
  globalButtonsArr.push(
    new Elem(procentScrollInputsArr.length, +newInput.value, 255, 255, 255, 1, 0)
  );
}
