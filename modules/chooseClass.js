class ChooseFunction {
  choose1(
    colorPanelButtonCoordinatsX,
    colorPanelButtonCoordinatsY,
    colorPanel,
    rgbNumber,
    colorTitle,
    transparencyScrollRgb
  ) {
    let coordinats1 = 255 - (colorPanelButtonCoordinatsY * 255) / colorPanel.offsetHeight;
    let coordinats2X =
      255 - (colorPanelButtonCoordinatsX * (255 - rgbNumber)) / colorPanel.offsetWidth;
    let coordinats2Y =
      ((colorPanel.offsetHeight - colorPanelButtonCoordinatsY) * coordinats2X) /
      colorPanel.offsetHeight;
    let coordinats3X = 255 - (colorPanelButtonCoordinatsX * 255) / colorPanel.offsetWidth;
    let coordinats3Y =
      ((colorPanel.offsetHeight - colorPanelButtonCoordinatsY) * coordinats3X) /
      colorPanel.offsetHeight;
    ...
    colorsExampleArr.find(
      (item) => +item.dataset.number === +elem.dataNumber
    ).style.background = `rgb(${coordinats1}, ${coordinats2Y}, ${coordinats3Y}`;
    // debugger
  }
  ...
  choose2(
    ...
    let coordinats1Y =
      ((colorPanel.offsetHeight - colorPanelButtonCoordinatsY) * coordinats1X) /
      colorPanel.offsetHeight;
    let coordinats3X = 255 - (colorPanelButtonCoordinatsX * 255) / colorPanel.offsetWidth;
    let coordinats3Y =
      ((colorPanel.offsetHeight - colorPanelButtonCoordinatsY) * coordinats3X) /
    ...
    colorsExampleArr.find(
      (item) => +item.dataset.number === +elem.dataNumber
    ).style.background = `rgb(${coordinats1Y}, ${coordinats2}, ${coordinats3Y}`;
  }
  ...
  choose4(
    ...
    colorsExampleArr.find(
      (item) => +item.dataset.number === +elem.dataNumber
    ).style.background = `rgb(${coordinats1Y}, ${coordinats2Y}, ${coordinats3}`;
  }
...
class ChooseFunction2 {
  choose1(colorPanel, elem) {
    ...
    let coordinats3X =
      (elem.coordinats3 * colorPanel.offsetHeight) / (colorPanel.offsetHeight - c1Y);
  ...
  choose2(colorPanel, elem) {
    ...
    let coordinats3X =
      (elem.coordinats3 * colorPanel.offsetHeight) / (colorPanel.offsetHeight - c2Y);
    let c3X = ((255 - coordinats3X) * colorPanel.offsetWidth) / 255;
  ...
  choose3(colorPanel, elem) {
    let c2Y = ((255 - elem.coordinats2) * colorPanel.offsetHeight) / 255;
    let coordinats1X =
      (elem.coordinats1 * colorPanel.offsetHeight) / (colorPanel.offsetHeight - c2Y);
  ...
  choose4(colorPanel, elem) {
    ...
    let coordinats1X =
      (elem.coordinats1 * colorPanel.offsetHeight) / (colorPanel.offsetHeight - c3Y);
    let c1X = ((255 - coordinats1X) * colorPanel.offsetWidth) / 255;
  ...
  choose5(colorPanel, elem) {
    let c3Y = ((255 - elem.coordinats3) * colorPanel.offsetHeight) / 255;
    let coordinats2X =
      (elem.coordinats2 * colorPanel.offsetHeight) / (colorPanel.offsetHeight - c3Y);
  ...
  choose6(colorPanel, elem) {
    let c1Y = ((255 - elem.coordinats1) * colorPanel.offsetHeight) / 255;
    let coordinats2X =
      (elem.coordinats2 * colorPanel.offsetHeight) / (colorPanel.offsetHeight - c1Y);
    let c2X = ((255 - coordinats2X) * colorPanel.offsetWidth) / 255;
  ...
  changeColorMetod(X, Y, elem) {
    ...
    transparencyScrollControlButton.style.left = `${
      elem.transparency * transparencyScroll.offsetWidth
    }px`;
    // debugger
  }
}
