const transparencyScrollControlButton = document.querySelector(
  ".transparency_scroll_control_button"
);
...
function scrollTransparencyPanel() {
  transparencyScroll.addEventListener("mousemove", (event) => {
    if (event.which === 1) {
      ...
      if (
        mouseСoordinatesTransparencyPanel > 0 &&
        mouseСoordinatesTransparencyPanel <= transparencyScroll.offsetWidth
      ) {
...
function changeTransparencyScroll(event) {
  ...
  transparencyScrollControlButton.style.left = `${
    mouseСoordinatesColorPanel - transparencyScrollControlButton.offsetWidth / 2
  }px`;
  let transparency = (
    +transparencyScrollControlButton.style.left
      .split("")
      .filter((item) => !isNaN(+item) || item === ".")
      .join("") / transparencyScroll.offsetWidth
  ).toFixed(2);
